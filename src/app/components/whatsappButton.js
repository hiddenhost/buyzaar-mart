...
const WhatsAppButton = ({ 
  ...
  showOnScroll = true,
  scrollThreshold = 100,
  size = "w-14 h-14", // Tailwind size classes
  showTooltip = true,
    ...
    <a
      ...
      className={`fixed ${
        position === "bottom-right"
          ...
          ? "top-28 right-6"
          : "top-28 left-6"
      } ${size} bg-[#b00000] hover:bg-[#8f0000] text-white rounded-full shadow-lg hover:shadow-xl cursor-pointer transition-all duration-300 transform hover:scale-110 flex items-center justify-center group`}
      aria-label="Call Us"
    >
      <Phone
        size={22}
    ...
    {showTooltip && isHovered && (
      <div
        className={`fixed ${getPositionClasses()
          .replace("bottom-6", "bottom-24")
          .replace(
            "top-6",
            "top-24"
          )} ${getTooltipPosition()} px-3 py-2 bg-gray-800 text-white text-sm rounded-lg shadow-lg whitespace-nowrap transform transition-all duration-200 opacity-100`}
      >
        <div className="relative">
          Chat with us on WhatsApp
          <div
            className={`absolute ${
              position.includes("bottom") ? "top-full" : "bottom-full"
            } ${
              position.includes("left") ? "left-4" : "right-4"
            } w-2 h-2 bg-gray-800 transform rotate-45 ${
              position.includes("bottom") ? "-mt-1" : "-mb-1"
            }`}
          ></div>
        </div>
      </div>
